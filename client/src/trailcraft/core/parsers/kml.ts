...
export function parseKml(xml: string, fileName: string): Track {
  ...
  // gx:Track 优先(带时间)
  const coords = Array.from(xml.matchAll(/<gx:coord>([^<]+)<\/gx:coord>/g))
  if (coords.length > 0) {
    const whens = Array.from(xml.matchAll(/<when>([^<]+)<\/when>/g))
    coords.forEach((c, i) => {
    ...
    for (const ls of Array.from(xml.matchAll(/<LineString\b[^>]*>([\s\S]*?)<\/LineString>/g))) {
...
export function parseKmlWaypoints(xml: string): KmlWaypoint[] {
  ...
  for (const pm of Array.from(xml.matchAll(PLACEMARK_RE))) {
